...
public class PDFsharpMigraDocReportAdapterTests
{
    ...
    public async Task Generate_TableAndTextPages_ReturnsAndSavesPdf()
    {
        ...
        var content = new ReportPageContentDto
        {
            Pages =
            [
                new ReportPageDataDto { Name = "Tabela", PageType = SmartDigitalPsicoAPI.Core.SDK.Domain.Enuns.EReportPageType.Table, Rows = [new SampleRow { Name = "Ana", Value = 3, Empty = null }], PropertiesToIgnore = ["Empty"] },
                new ReportPageDataDto { Name = "Texto", PageType = SmartDigitalPsicoAPI.Core.SDK.Domain.Enuns.EReportPageType.Text, Rows = [new SampleRow { Name = "Bruno", Value = 8, Empty = null }], PropertiesToIgnore = ["Empty"] }
            ]
        };
