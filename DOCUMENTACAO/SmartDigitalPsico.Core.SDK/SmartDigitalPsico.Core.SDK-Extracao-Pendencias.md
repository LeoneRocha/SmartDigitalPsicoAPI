# SmartDigitalPsico.Core.SDK — Levantamento e plano das extrações pendentes
...
- [SmartDigitalPsico.Core.SDK-Substituicao.md](./SmartDigitalPsico.Core.SDK-Substituicao.md) (v1.4 — substituição de tipos leves concluída)
- [SmartDigitalPsico.Core.SDK-MigracaoGenericos.md](./SmartDigitalPsico.Core.SDK-MigracaoGenericos.md) (v1.6 — genéricos pesados consolidados no NuGet único)
...
As iniciativas anteriores centralizaram no `SmartDigitalPsico.Core.SDK` os tipos genéricos leves (entidades base, interfaces, DTOs comuns, helpers) e as implementações pesadas (EF, Dapper, Redis, Mongo, Cosmos, Azure), removendo os shims `SCH_MIGR_*` e `SCH_MIG_GEN_*`. Porém, uma nova varredura completa de `Implementations` (Domain, Service, Infrastructure) revelou que:
...
| Regra | Descrição |
| ----- | --------- |
| **Centralizar o genérico** | Toda implementação genérica/reutilizável tem fonte única no NuGet `SmartDigitalPsico.Core.SDK`. |
| **Manter o específico** | Entidades de produto, repositórios de domínio, `SmartDigitalPsicoDataContext`, EF configs, seed, middlewares de composição e validators de regra de negócio permanecem em `Implementations`. |
...
| **Build obrigatório por lote** | `dotnet build SmartDigitalPsicoAPI.sln -c Release` verde ao fim de cada lote. |
| **Testes preservados e replicados** | Testes dos tipos migrados continuam nos projetos originais (enquanto o tipo existir) e são replicados/adaptados em `SmartDigitalPsico.Core.SDK.Tests`. |
...
| # | Tema | Decisão |
| - | ---- | ------- |
| 1 | **FluentValidation** | **Não entra no Core.SDK** (decisão reconfirmada em 2026-07-14). Os *DTOs* de validação genérica movem para o SDK; os *validators* FluentValidation permanecem em `SmartDigitalPsico.Domain`. |
...
| 4 | **`AuditableBaseEntity`** | Redesign em camadas: o SDK ganha `AuditableEntityBase` (ids `long` + datas, **sem** navegação `User`); o Domain mantém `AuditableBaseEntity : AuditableEntityBase` adicionando apenas as navigation properties `User?`. Nenhuma coluna/migration muda. |
| 5 | **`GenericService<TEntity>`** | A implementação **permanece** em `SmartDigitalPsico.Service` (depende dos validators FluentValidation — consequência da decisão 1). O **contrato** `IGenericService<TEntity>` move para o SDK. |
...
flowchart TB
  ...
  Core[SmartDigitalPsico.Core.SDK]
...
> Ação padrão: migrar consumidores para o tipo `SmartDigitalPsico.Core.SDK.*` → deletar o original → build + testes. Sem shim (decisão §2.7).

### 3.1 SmartDigitalPsico.Domain
...
> Ação padrão: mover arquivo para o Core.SDK (namespace `SmartDigitalPsico.Core.SDK.*`), migrar consumidores, replicar testes em `Core.SDK.Tests`. Nenhum `PackageReference` novo.
...
| ------- | ---- | ------- | ------------ | ------------ |
| `Interfaces/Services/Generic/IGenericService.cs` | `IGenericService<TEntity>` | `Domain/Interfaces/Services/Generic/` | Já usa só tipos SDK (`ServiceResponse`, `EntityBase`, `IUserContext`) + `System.Linq.Expressions` | Todas as interfaces de service de feature (`IApplicationService`, `IUserService`, `ITenantService`, `IPlanService`, `IApplicationConfigurationService`, `ICloudConfigurationService`, `IApplicationPlanSubscriptionService`, `IApiKeyTokenService`, `IBillingEventService`) + `GenericService` (Service) |
...
| ------- | --------------- | ------------------------- |
| `DTOs/Common/GenericValidationDtos.cs` | `GenericEntityIdValidationDto`, `GenericPredicateValidationDto`, `GenericEntitiesValidationDto`, `GenericIdsValidationDto`, `GenericEntityUpdateValidationDto` (dependem só de `EntityBase` do SDK) | — |
...
> Os validators FluentValidation correspondentes (`GenericValidationDtoValidators.cs`, `InternalGuardValidators.cs`) **permanecem no Domain** (§2.1) e passam a referenciar os DTOs pelo namespace do SDK. Manter os error codes atuais (`SmartDigitalPsico.Domain.*`) para não quebrar contrato de resposta.
...
| `Domain/DependeciesCollection/SharedDependeciesCollection.cs` | `ISharedDependeciesCollection`, `SharedDependeciesCollection` | idem | `Microsoft.Extensions.Configuration.Abstractions` + abstrações SDK (`IAppLogger`, `IAppMapper`) | `WebApplicationBuilderServicesConfigure`, `AuthenticationService`, testes |
...
| `AutoMapperAdapter` | `Domain/Mappings/AutoMapperAdapter.cs` | `AutoMapper` | Adapter fino (2 métodos) sobre `IMapper` implementando o `IAppMapper` do SDK. `AutoMapperProfile` (mapeamentos de feature) **fica** no Domain. `AddAutoMapperProviders` (Service) passa a registrar o adapter do SDK. |
...
| `SmartDigitalPsicoDataBaseConnectionFactory` → `EfDbConnectionFactory<TContext>` | `Infrastructure/Data/SmartDigitalPsicoDataBaseConnectionFactory.cs` | nenhuma nova (EF já condicional) | Generalizar como factory genérica por `TContext` implementando `ISmartDigitalPsicoDataBaseConnectionFactory` do SDK; consolidar a lógica de pooling MySQL duplicada em `ApiPerformanceExtensions` (Service). O host mantém uma subclasse/registro fechando `TContext = SmartDigitalPsicoDataContext`. |
...
| ---- | ------ | -------- | -------- |
| `AuditableBaseEntity` | `Domain/Entities/Common/AuditableBaseEntity.cs` | Navigation properties `User?` (entidade de produto) | SDK ganha `AuditableEntityBase : EntityBase` com `CreatedUserId`/`ModifyUserId` (`long?`) + datas, **sem** navs. Domain mantém `AuditableBaseEntity : AuditableEntityBase` adicionando somente `User? CreatedUser` / `User? ModifyUser`. Colunas/mapeamentos EF idênticos → gate EF com Up/Down vazios obrigatório. Consumidores (`FileExportHistory`, `ApplicationLanguage`) não mudam. |
...
| `SerilogAdapter` | Movido para `SmartDigitalPsico.Core.SDK.Infrastructure.Logging` (banda net8/net10); o host apenas registra no DI. |
| `AutoMapperProfile` | Mapeamentos de DTOs/entidades de produto. |
| `SmartDigitalPsicoDataContext`, EF configs, seed, migrations, repositórios de domínio (EF e Dapper), `GetActiveNonExpiredTokensRoutineDefinition` | Específicos do produto (regra herdada). |
...
flowchart TD
  ...
  L5[Lote 5 - Grupo D AuditableEntityBase]
...
### Lote 5 — Grupo D (`AuditableEntityBase`)

- [ ] Criar `AuditableEntityBase` no SDK (ids `long?` + datas, sem navs).
...
- [ ] Atualizar `SmartDigitalPsico.Core.SDK/README.md` (catálogo de tipos novos).
- [ ] Banner de complemento nos docs de `Documentation/Features/FEITOS/SmartDigitalPsico.Core.SDK/` apontando para este documento.
...
- [ ] `dotnet test SmartDigitalPsicoAPI.sln -c Release --no-build -m:1` — 0 falhas; contagem ≥ baseline (~3231+).
...
rg "SmartDigitalPsico.Domain.Common.ProcessStopwatch|SmartDigitalPsico.Domain.Helpers.CultureDateTimeHelper" -g "*.cs"
rg "SmartDigitalPsico.Domain.Interfaces.Dapper.RepositoryImplementationKind" -g "*.cs"
rg "SmartDigitalPsico.Domain.Sanitization.RichContentFormat|SmartDigitalPsico.Domain.Sanitization.RichContentSanitizeOptions" -g "*.cs"
rg "SmartDigitalPsico.Data.Caching.Providers.MemoryCacheProvider" -g "*.cs"
rg "namespace SmartDigitalPsico.Data.Data.Configurations" -g "DatabaseProvider*.cs"
...
| `AuditableEntityBase` alterar schema | Propriedades/colunas idênticas; gate EF §8.2 com Up/Down vazios obrigatório no Lote 5. |
...
| 4 | Grupo D: `AuditableEntityBase` no SDK; gate EF com Up/Down vazios. |
