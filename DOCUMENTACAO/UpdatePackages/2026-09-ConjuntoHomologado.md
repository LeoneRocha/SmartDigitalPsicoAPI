# Conjunto Homologado — Ciclo 2026-09 (Governança e Homologação NuGet .NET 10)
...
### Bloco A — Plataforma .NET 10 (patch alinhado 10.0.12)

Todos os pacotes de plataforma ASP.NET Core e Microsoft.Extensions foram atualizados de forma coesa para o patch **10.0.12**:

| Pacote | Versão Anterior | Versão Aplicada | Latest NuGet | Observações |
| ------ | --------------- | --------------- | ------------ | ----------- |
| `Microsoft.AspNetCore.Authentication.JwtBearer` | 10.0.11 | **10.0.12** | 10.0.12 | Estável / Patch .NET 10 |
| `Microsoft.AspNetCore.Mvc.Testing` | 10.0.11 | **10.0.12** | 10.0.12 | Estável / Patch .NET 10 |
| `Microsoft.Extensions.Caching.Memory` | 10.0.11 | **10.0.12** | 10.0.12 | Estável / Patch .NET 10 |
| `Microsoft.Extensions.Configuration.FileExtensions` | 10.0.11 | **10.0.12** | 10.0.12 | Estável / Patch .NET 10 |
| `Microsoft.Extensions.Configuration.Json` | 10.0.11 | **10.0.12** | 10.0.12 | Estável / Patch .NET 10 |
| `Microsoft.Extensions.Hosting` | 10.0.11 | **10.0.12** | 10.0.12 | Estável / Patch .NET 10 |
| `Microsoft.Extensions.Hosting.WindowsServices` | 10.0.11 | **10.0.12** | 10.0.12 | Estável / Patch .NET 10 |
| `Microsoft.Extensions.Identity.Core` | 10.0.11 | **10.0.12** | 10.0.12 | Estável / Patch .NET 10 |

---

### Bloco B — Persistência (EF Core 9.0.20 + Pomelo 9 — Trava Pomelo Mantida)

> [!IMPORTANT]
> **Trava de Grafo Pomelo ↔ EF Core 10:** O provider `Pomelo.EntityFrameworkCore.MySql 9.0.0` continua como a versão oficial mais recente no NuGet.org e exige EF Core <= 9.x (`[9.0.0, 9.0.999]`). A subida para `Microsoft.EntityFrameworkCore 10.x` permanece travada neste ciclo. Todos os pacotes `Microsoft.EntityFrameworkCore.*` foram promovidos para o patch mais recente da linha 9.x (**`9.0.20`**).

| Pacote | Versão Anterior | Versão Aplicada | Latest NuGet | Justificativa se != latest |
| ------ | --------------- | --------------- | ------------ | -------------------------- |
| `Microsoft.EntityFrameworkCore` (7 pacotes) | 9.0.18 | **9.0.20** | 10.0.12 | Pomelo oficial ainda sem release estável 10.x para EF Core 10 |
| `Pomelo.EntityFrameworkCore.MySql` | 9.0.0 | **9.0.0** | 9.0.0 | Latest oficial compatível com EF Core 9 |
| `Microsoft.Data.SqlClient` | 6.1.6 | **6.1.6** | 7.0.2 | Mantido estável no range EF 9 (versão 7.x traz breaking changes TLS) |
| `SQLitePCLRaw.bundle_e_sqlite3` | 3.0.5 | **3.0.5** | 3.0.5 | Override transitivo de segurança mantido |
...
| `HtmlSanitizer` | 9.2.1039 | **9.2.1039** | 9.2.1039 | Estável |
| `AngleSharp` | 1.7.2 | **1.8.0** | 1.8.0 | Atualização de minor estável |
| `AngleSharp.Css` | 1.0.2 | **1.1.0** | 1.1.0 | Atualização de minor estável |
...
| `QuestPDF` | 2026.7.3 | **2026.8.0** | 2026.8.0 | Atualização de minor estável |
...
| `SmartCoreHub.Core.SDK` | 20260909.1825.0 | **20260909.1825.0** | 20260909.1825.0 | Build oficial nuget.org |
...
| Pacote | Versão Anterior | Versão Aplicada | Latest NuGet | Justificativa se != latest |
| ------ | --------------- | --------------- | ------------ | -------------------------- |
| `Microsoft.NET.Test.Sdk` | 18.9.0 | **18.10.0** | 18.10.0 | Atualização estável com VSTest / NUnit |
| `NUnit` | 4.6.1 | **4.6.1** | 4.6.1 | Estável |
| `NUnit3TestAdapter` | 6.2.0 | **6.3.0** | 6.3.0 | Atualização estável do adapter |
...
| `AwesomeAssertions` | 9.6.0 | **9.6.0** | 9.6.0 | Estável |
...
backend:
  ...
  dotnet test SmartDigitalPsicoAPI.sln -c Release  -> 1.343 aprovados / 0 falhas:
    - SmartDigitalPsico.Domain.Test:            569 aprovados (0 falhas)
    - SmartDigitalPsico.Service.Test:           448 aprovados (0 falhas)
    - SmartDigitalPsico.Core.SDK.Tests:         140 aprovados (0 falhas)
  ...
  Total: 1.343 aprovados (100% de sucesso)

  dotnet ef migrations add ValidacaoPosUpdateTemp -> Up() e Down() vazios (0 drift DDL)
```
...
1. **EF Core 10 (Conjunto v2):** destravar quando `Pomelo.EntityFrameworkCore.MySql` 10.x oficial publicar versão final no nuget.org; promover em conjunto com `Moq.EntityFrameworkCore 10.x`.
...
3. **Microsoft.Data.SqlClient 7.x:** Avaliar adoção em ciclo coordenado quando o time homologar os novos requisitos de criptografia estrita TLS do driver SQL Server.
