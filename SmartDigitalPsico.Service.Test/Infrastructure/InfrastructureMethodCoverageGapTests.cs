...
{
    // Cenário: SmartDigitalPsicoAPI.Core.SDK.Service.Infrastructure.StorageQueueService encaminha operações ao repositório criado pela factory.
    // Objetivo: cobrir Enqueue/Dequeue/Delete do wrapper.
    [Test]
    public async Task StorageQueueService_AllOperations_ForwardToRepository()
    {
        // Arrange
        var repo = new Mock<SmartDigitalPsicoAPI.Core.SDK.Domain.Interfaces.Infrastructure.IStorageQueueContract>();
        repo.Setup(x => x.DequeueMessageAsync()).ReturnsAsync("payload");
        var factory = new Mock<SmartDigitalPsicoAPI.Core.SDK.Domain.Interfaces.Infrastructure.IStorageQueueRepositoryFactory>();
        factory.Setup(x => x.Create(global::SmartDigitalPsicoAPI.Core.SDK.Domain.Enuns.EStorageAdapterType.Azure, "q1")).Returns(repo.Object);
        var sut = new SmartDigitalPsicoAPI.Core.SDK.Service.Infrastructure.StorageQueueService(factory.Object, "q1");
    ...
    // Cenário: SendNotificationService e global::SmartDigitalPsicoAPI.Core.SDK.Service.Infrastructure.Smtp.EmailContext/SmartDigitalPsicoAPI.Core.SDK.Service.Infrastructure.Notification.EmailService orquestram envio.
    // Objetivo: cobrir wrappers de notificação e e-mail.
    [Test]
    public async Task NotificationAndEmailWrappers_Send_InvokeDependencies()
    {
        // Arrange
        var platform = new Mock<global::SmartDigitalPsicoAPI.Core.SDK.Domain.Interfaces.Notification.INotificationPlatformService>();
        ...
        var strategy = new Mock<global::SmartDigitalPsicoAPI.Core.SDK.Domain.Interfaces.Smtp.IEmailStrategy>();
        var strategyFactory = new Mock<global::SmartDigitalPsicoAPI.Core.SDK.Domain.Interfaces.Smtp.IEmailStrategyFactory>();
        strategyFactory.Setup(x => x.CreateStrategy(global::SmartDigitalPsicoAPI.Core.SDK.Domain.Enuns.EEmailStrategyType.Smtp)).Returns(strategy.Object);
        var emailContext = new global::SmartDigitalPsicoAPI.Core.SDK.Service.Infrastructure.Smtp.EmailContext(strategyFactory.Object);
        var emailService = new SmartDigitalPsicoAPI.Core.SDK.Service.Infrastructure.Notification.EmailService(emailContext);
        var template = new global::SmartDigitalPsicoAPI.Core.SDK.Domain.VO.DataNotificationTemplateVO
        {
            Subject = "S",
            Body = "Hello {Name}",
            ToEmails = ["a@test.com"]
        ...
        strategy.Verify(x => x.SendEmailAsync(It.Is<global::SmartDigitalPsicoAPI.Core.SDK.Domain.DTO.SMTP.EmailMessageDto>(m => m.Subject == "S" && m.ToEmails.Contains("a@test.com"))), Times.Once);
    }
    ...
    public void SmtpEmailStrategy_SslAlwaysEnabled_ThrowsOnUnreachableServer()
    {
        ...
        // Act
        var action = () => strategy.SendEmailAsync(new global::SmartDigitalPsicoAPI.Core.SDK.Domain.DTO.SMTP.EmailMessageDto
        {
            Subject = "Hi",
            Message = "<b>ok</b>",
    ...
    public async Task AzureStorageBlobAdapter_CannotGenerateSas_ReturnsEmpty()
    {
        ...
        var sut = new SmartDigitalPsicoAPI.Core.SDK.Service.Infrastructure.Azure.Storage.AzureStorageBlobAdapter(config, anonymous);
