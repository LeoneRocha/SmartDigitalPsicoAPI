...
{
    // Cenário: StorageQueueService encaminha operações ao repositório criado pela factory.
    // Objetivo: cobrir Enqueue/Dequeue/Delete do wrapper.
    [Test]
    public async Task StorageQueueService_AllOperations_ForwardToRepository()
    {
        // Arrange
        var repo = new Mock<IStorageQueueContract>();
        repo.Setup(x => x.DequeueMessageAsync()).ReturnsAsync("payload");
        var factory = new Mock<IStorageQueueRepositoryFactory>();
        factory.Setup(x => x.Create(EStorageAdapterType.Azure, "q1")).Returns(repo.Object);
        var sut = new StorageQueueService(factory.Object, "q1");
    ...
    // Cenário: SendNotificationService e EmailContext/EmailService orquestram envio.
    // Objetivo: cobrir wrappers de notificação e e-mail.
    [Test]
    public async Task NotificationAndEmailWrappers_Send_InvokeDependencies()
    {
        // Arrange
        var platform = new Mock<INotificationPlatformService>();
        ...
        var strategy = new Mock<IEmailStrategy>();
        var strategyFactory = new Mock<IEmailStrategyFactory>();
        strategyFactory.Setup(x => x.CreateStrategy(EEmailStrategyType.Smtp)).Returns(strategy.Object);
        var emailContext = new EmailContext(strategyFactory.Object);
        var emailService = new EmailService(emailContext);
        var template = new DataNotificationTemplateVO
        {
            Subject = "S",
            Body = "Hello {Name}",
            ToEmails = ["a@test.com"]
    ...
    public async Task AzureStorageBlobAdapter_CannotGenerateSas_ReturnsEmpty()
    {
        ...
        var sut = new AzureStorageBlobAdapter(config, anonymous);
