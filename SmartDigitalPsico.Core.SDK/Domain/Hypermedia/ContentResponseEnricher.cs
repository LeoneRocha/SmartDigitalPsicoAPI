...
/// <summary>
/// Enricher HATEOAS — retenção lógica local (ServiceResponse / ISupportsHyperMedia SDP).
/// </summary>
[SdkWrappedSource(
    targetType: "SmartCoreHub.Core.SDK.Domain.Hypermedia.ContentResponseEnricher`1",
    targetPackage: "SmartCoreHub.Core.SDK",
    description: "Espelho documentado; impl local por ServiceResponse/ISupportsHyperMedia SDP.")]
public abstract class ContentResponseEnricher<T> : IResponseEnricher where T : ISupportsHyperMedia
{
    ...
    public async Task Enrich(ResultExecutingContext context)
    {
        ...
        if (context.Result is OkObjectResult okObjectResult)
        {
            await HandleOkObjectResult(okObjectResult, urlHelper).ConfigureAwait(false);
        }
    }
    ...
    private async Task HandleOkObjectResult(OkObjectResult okObjectResult, IUrlHelper urlHelper)
    {
        switch (okObjectResult.Value)
        {
            case ServiceResponse<T> serviceResponse:
                await HandleServiceResponse(serviceResponse, urlHelper).ConfigureAwait(false);
                break;
            case ServiceResponse<List<T>> serviceResponseList:
                HandleServiceResponseList(serviceResponseList, urlHelper);
                break;
            case T model:
                await EnrichModel(model, urlHelper).ConfigureAwait(false);
                break;
