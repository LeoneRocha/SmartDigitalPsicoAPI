        ...
        public async Task<ServiceResponse<bool>> AddItemToBatchAsync(string batchToken, AddScheduleItemDto item)
        {
            ...
            try
            {
                ...
                var overlapContext = new ScheduleItemValidationContext
                {
                    ...
                    ExistingItems = batch.ScheduleData,
                    MedicalId = batch.MedicalId
                };

                var overlapValidationResult = await _validators.ScheduleItemOverlapValidator.ValidateAsync(overlapContext);
                if (!overlapValidationResult.IsValid)
                {
                    response.Success = false;
                    response.Errors = HelperValidation.ConvertValidationFailureListToErroResponse(overlapValidationResult.Errors);
                    response.Message = overlapValidationResult.Errors.First().ErrorMessage;
                    return response;
                }
